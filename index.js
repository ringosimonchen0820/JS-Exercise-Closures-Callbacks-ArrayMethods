...
*/
function processLength(arr, arrLength) {
...
*/
function processLastItem(stringList, lastItem) {
  return lastItem(stringList[stringList.length - 1]);
}
 ...
 * should return 1000.
*/
function processSum(numberList, sum) {
  return sum(numberList.reduce((a, b) => {return a + b},0));
}
 ...
 * should return 1000.
*/
function processProduct(num1, num2, product) {
 ...
 * should return "sad".
*/
function processContains(item, list, exists) {
  return exists(list.includes(item));
}
 ...
 * should return 3.
*/
function processDuplicateFree(arr, noDuplicate) {
  return noDuplicate(arr.filter(function (item, index) {return arr.indexOf(item) === index}))
}
...
function getFullNames(runners) {
  ...
  runners.forEach(function(names) {
    nameList.push(`${names.last_name}, ${names.first_name}`);
  })
...
function tallyUpDonations(runners) {
  return runners.reduce(function(a, currentValue) {
    return a + currentValue.donation;
  }, 0);
}
 ...
 * etc
*/
function counterMakerWithLimit(limits) {
  var count = 0;
  return function() {
    if(count > limits) {
      count = 0;
